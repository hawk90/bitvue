export const meta = {
  ...
  whenToUse:
    'Run after the anti-pattern catalog is stable (see docs/anti-patterns/INDEX.md). args: { files?: string[] } - category IDs like ["PARSE","MEM"] to scope the scan; omit for all files (expensive - 47 files as of the last catalog wave, one agent per file).',
...
const AREA_HINTS = {
  ...
  IPC: 'crates/bitvue-sidecar/src frontend/services/electronBridgeService.ts',
  PIXEL: 'crates/bitvue-decode crates/bitvue-metrics',
  ERR: 'crates/ crates/bitvue-sidecar/src',
  ...
  UIX_ASYNC: 'frontend/ crates/bitvue-sidecar/src',
  ...
  UIX_ERR: 'frontend/ crates/bitvue-sidecar/src',
  UIX_A11Y: 'frontend/',
  TAURI_CMD: 'crates/bitvue-sidecar/src',
  TAURI_EVT: 'crates/bitvue-sidecar/src frontend/',
  TAURI_WEB: 'bitvue-desktop/electron',
  ...
  RPERF: 'crates/',
  SER: 'crates/bitvue-sidecar/src crates/bitvue-cli crates/bitvue-mcp',
  PLUGIN: 'crates/bitvue-codecs crates/bitvue-codecs-parser Cargo.toml',
  MCP: 'crates/bitvue-mcp crates/bitvue-core/src/mcp.rs',
  BUILD: 'Cargo.toml crates/*/Cargo.toml .github/workflows/ci.yml bitvue-desktop/package.json',
  PLAT: 'bitvue-desktop/electron frontend/',
  TEST: 'crates/*/tests fuzz/ scripts/ crates/bitvue-cli/tests',
  OBS: 'crates/bitvue-core crates/bitvue-sidecar/src',
  SEC: 'crates/bitvue-sidecar/src crates/bitvue-formats crates/bitvue-core',
}
  ...
  fileKey =>
    agent(
      `Bitvue project (video bitstream analyzer, Tauri + Rust + React). Audit docs/anti-patterns/${fileKey}.md ` +
        ...
        `actually present in the code. Grep/read around ${AREA_HINTS[fileKey] ?? 'crates/ crates/bitvue-sidecar/src frontend/'} ` +
