...
impl FilmstripPanel {
    ...
    pub(super) fn render_frame_sizes_view(
        ...
        let _i_avg = if !i_frames.is_empty() {
            i_total / i_frames.len()
        } else {
            0
        };
        let _p_avg = if !p_frames.is_empty() {
            p_total / p_frames.len()
        } else {
            0
        };
        let _b_avg = if !b_frames.is_empty() {
            b_total / b_frames.len()
        } else {
            0
        };
        ...
        let max_size_kbps = frames
            .iter()
            .map(|f| f.size as f32 * 8.0 / 1000.0)
            .fold(0.0f32, f32::max);
        let max_avg = moving_avg.iter().cloned().fold(0.0f32, f32::max);
        let y_max = max_size_kbps.max(max_avg);

        tracing::info!(
            "📊 Frame Sizes: max_size_kbps={}, max_avg={}, y_max={}",
            max_size_kbps,
            max_avg,
            y_max
        );
        ...
        let _graph_response = egui::ScrollArea::horizontal()
            ...
            .show(ui, |ui| {
                ...
                let total_width =
                    frames.len() as f32 * (bar_width + bar_spacing) + y_label_width + qp_axis_width;
                ...
                let y_axis_rect =
                    Rect::from_min_size(full_rect.min, Vec2::new(y_label_width, graph_height));
                painter.rect_filled(
                    y_axis_rect,
                    Rounding::ZERO,
                ...
                let title_rect = Rect::from_min_size(full_rect.min, Vec2::new(y_label_width, 22.0));
                ...
                for i in (0..frames.len()).step_by(50) {
                    let x =
                        graph_rect.left() + i as f32 * (bar_width + bar_spacing) + bar_width / 2.0;
                    painter.text(
                ...
                painter.text(
                    egui::pos2(
                        graph_rect.right() + qp_axis_width / 2.0,
                        full_rect.top() + 11.0,
                    ),
                ...
                if self.show_moving_avg {
                    tracing::info!(
                        "📊 Drawing moving average line: {} points, y_max={}",
                        moving_avg.len(),
                        y_max
                    );
                    if y_max > 0.0 {
                        ...
                        for (idx, &avg_size) in moving_avg.iter().enumerate() {
                            let x = graph_rect.left()
                                + idx as f32 * (bar_width + bar_spacing)
                                + bar_width / 2.0;
                ...
                tracing::info!(
                    "📊 Drew {} visible bars out of {} total frames",
                    visible_bars,
                    frames.len()
                );
                        ...
                        .show(ui, |ui| {
                            ...
                            ui.horizontal(|ui| {
                                ...
                                if ui
                                    .selectable_label(
                                        self.show_i_frames,
                                        egui::RichText::new("Intra slices").size(9.0),
                                    )
                                    .clicked()
                                {
                            ...
                            ui.horizontal(|ui| {
                                ...
                                if ui
                                    .selectable_label(
                                        self.show_p_frames,
                                        egui::RichText::new("P slices").size(9.0),
                                    )
                                    .clicked()
                                {
                            ...
                            ui.horizontal(|ui| {
                                ...
                                if ui
                                    .selectable_label(
                                        ...
                                        egui::RichText::new("Moving Avg").size(9.0),
                                    )
                                    .clicked()
                                {
                            ...
                            ui.label(
                                egui::RichText::new("Moving Average parameters")
                                    .strong()
                                    .size(9.0),
                            );
                            ui.add_space(3.0);
                            ...
                            ui.label(
                                egui::RichText::new(format!("Total: {} frames", frames.len()))
                                    .size(8.5),
                            );
                            ui.label(
                                egui::RichText::new(format!(
                                    "I: {} ({:.0}%)",
                                    i_frames.len(),
                                    i_frames.len() as f32 / frames.len() as f32 * 100.0
                                ))
                                .size(8.5),
                            );
                            ui.label(
                                egui::RichText::new(format!(
                                    "P: {} ({:.0}%)",
                                    p_frames.len(),
                                    p_frames.len() as f32 / frames.len() as f32 * 100.0
                                ))
                                .size(8.5),
                            );
                        });
