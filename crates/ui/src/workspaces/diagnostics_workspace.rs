...
use bitvue_core::event::{Diagnostic, Severity, Category};
use egui;
            ...
            .show(ui, |ui| {
                TableBuilder::new(ui)
                    ...
                    .header(20.0, |mut header| {
                        ...
                        header.col(|ui| {
                            ui.strong("Frame #").on_hover_text("Frame number (bitvue extension)");
                        });
                        header.col(|ui| {
                            ui.strong("Time").on_hover_text("Timestamp (bitvue extension)");
                        });
                        header.col(|ui| {
                            ui.strong("Pos");
                        });
                        ...
                        header.col(|ui| {
                            ui.strong("Count").on_hover_text("Repetition count (bitvue extension)");
                        });
                        header.col(|ui| {
                            ui.strong("Impact").on_hover_text("Impact score 0-100 (bitvue extension)");
                        });
                        header.col(|ui| {
                            ui.strong("Message");
                        });
                        ...
                        body.row(row_height, |mut row| {
                            ...
                            row.col(|ui| {
                                let (text, color) = match diagnostic.severity {
                                    Severity::Fatal => ("Fatal", egui::Color32::from_rgb(200, 50, 50)),
                                    Severity::Error => ("Error", egui::Color32::from_rgb(255, 80, 80)),
                                    Severity::Warn => ("Warning", egui::Color32::from_rgb(255, 200, 80)),
                                    Severity::Info => ("Info", egui::Color32::from_rgb(100, 150, 255)),
                                };
                            ...
                            row.col(|ui| {
                                let (icon, color) = if diagnostic.impact_score >= 80 {
                                    ("●", egui::Color32::from_rgb(255, 80, 80))  // Red dot - Critical
                                } else if diagnostic.impact_score >= 50 {
                                    ("▲", egui::Color32::from_rgb(255, 200, 80))  // Orange triangle - Warning
                                } else {
                                    ("○", egui::Color32::from_rgb(100, 200, 100))  // Green circle - OK
                                };
                                ui.horizontal(|ui| {
                            ...
                            row.col(|ui| {
                                let response = ui.selectable_label(false, &diagnostic.message);
                                if response.clicked() {
                                    clicked_diagnostic = Some((*diagnostic).clone());
                                }
                                response.on_hover_text("Click to navigate to error location");
                            });
                        });
                    }
                }
