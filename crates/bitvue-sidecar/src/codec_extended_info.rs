...
struct GetCodecExtendedInfoParams {
    frame_index: usize,
}

/// AV1 reference-frame lists (L0/L1) + QP histogram for one frame of stream A -- see this
/// module's doc. Control-only, same reasoning as `frame_analysis::get_frame_analysis_command`.
pub fn get_codec_extended_info_command(
    ...
    let params: GetCodecExtendedInfoParams = match serde_json::from_value(request.params.clone()) {
        Ok(p) => p,
        Err(err) => {
    ...
    match get_codec_extended_info(data, params.frame_index) {
