...
//! itself (not introduced here): `include_screenshots`/`include_interaction_trace`/
//! `include_logs` request flags exist but nothing captures screenshots, interaction traces, or
//! logs yet (see `evidence.rs`'s own doc); `render_snapshots` (per-panel render state) requires
//! frontend capture infrastructure that doesn't exist yet, so this command always passes an empty
//! slice -- the bundle's `render_snapshots/` directory is simply omitted, which
//! `export_evidence_bundle` already handles via its own `!render_snapshots.is_empty()` guard.
//!
...
pub fn export_evidence_bundle_command(core: &Core, request: &Request) -> Response {
    ...
    let result = export_evidence_bundle(&bundle_request, &[]);
    if !result.success {
        return Response::failure(
            request.id,
            WireError {
