...
pub fn build_syntax_tree(stream: &Vp9Stream) -> SyntaxNode {
    ...
    root.add_child(SyntaxNode::field(
        "Frame Count",
        stream.frames.len().to_string(),
    ));
    root.add_child(SyntaxNode::field(
        "Key Frames",
        stream.key_frames().len().to_string(),
    ));
...
fn build_frame_tree(index: usize, header: &FrameHeader) -> SyntaxNode {
    let frame_type = if header.is_key_frame() {
        "Key"
    } else {
        "Inter"
    };
    let show = if header.show_frame {
        ", show"
    } else {
        ", hidden"
    };
    ...
    let mut node = SyntaxNode::new(
        format!(
            "[{}] {} frame{} ({}x{})",
            index, frame_type, show, header.width, header.height
        ),
    ...
    node.add_child(SyntaxNode::field(
        "frame_type",
        format!("{:?}", header.frame_type),
    ));
    node.add_child(SyntaxNode::field(
        "show_frame",
        header.show_frame.to_string(),
    ));
    node.add_child(SyntaxNode::field(
        "error_resilient_mode",
        header.error_resilient_mode.to_string(),
    ));
    ...
    if header.render_width != header.width || header.render_height != header.height {
        size_node.add_child(SyntaxNode::field(
            "render_width",
            header.render_width.to_string(),
        ));
        size_node.add_child(SyntaxNode::field(
            "render_height",
            header.render_height.to_string(),
        ));
    }
    ...
    if !header.is_key_frame() && !header.intra_only {
        ...
        ref_node.add_child(SyntaxNode::field(
            "ref_frame_idx[0] (LAST)",
            header.ref_frame_idx[0].to_string(),
        ));
        ref_node.add_child(SyntaxNode::field(
            "ref_frame_idx[1] (GOLDEN)",
            header.ref_frame_idx[1].to_string(),
        ));
        ref_node.add_child(SyntaxNode::field(
            "ref_frame_idx[2] (ALTREF)",
            header.ref_frame_idx[2].to_string(),
        ));
        ref_node.add_child(SyntaxNode::field(
    ...
    quant_node.add_child(SyntaxNode::field(
        "base_q_idx",
        header.quantization.base_q_idx.to_string(),
    ));
    quant_node.add_child(SyntaxNode::field(
        "lossless",
        header.quantization.lossless.to_string(),
    ));
    if header.quantization.delta_q_y_dc != 0 {
        quant_node.add_child(SyntaxNode::field(
            "delta_q_y_dc",
            header.quantization.delta_q_y_dc.to_string(),
        ));
    }
    ...
    lf_node.add_child(SyntaxNode::field(
        "level",
        header.loop_filter.level.to_string(),
    ));
    lf_node.add_child(SyntaxNode::field(
        "sharpness",
        header.loop_filter.sharpness.to_string(),
    ));
    ...
    if header.segmentation.enabled {
        ...
        seg_node.add_child(SyntaxNode::field(
            "update_map",
            header.segmentation.update_map.to_string(),
        ));
        seg_node.add_child(SyntaxNode::field(
            "update_data",
            header.segmentation.update_data.to_string(),
        ));
    ...
    if header.num_tiles() > 1 {
        ...
        tile_node.add_child(SyntaxNode::field(
            ...
            format!(
                "{}x{} = {}",
                header.num_tile_cols(),
                header.num_tile_rows(),
                header.num_tiles()
            ),
        ));
