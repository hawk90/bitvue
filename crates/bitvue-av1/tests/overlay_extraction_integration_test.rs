...
use bitvue_av1::overlay_extraction::{
...
fn test_qp_grid_extraction_with_real_av1() {
    // Parse real AV1 data
    let obu_data = parse_ivf_to_obu_data("test_data/av1_test.ivf")
        .expect("Failed to load test AV1 file");
    ...
    // Extract QP grid
    let base_qp = 128i16;
    let qp_grid = extract_qp_grid_from_parsed(&parsed, 0, base_qp)
        .expect("Failed to extract QP grid");
    ...
    for &qp in &qp_grid.qp {
        assert!(qp >= 0 && qp <= 255, "QP value {} should be in [0, 255]", qp);
    }

    println!("QP Grid: {}x{}, {} blocks, avg QP: {:.1}",
        qp_grid.grid_w, qp_grid.grid_h, qp_grid.qp.len(),
...
fn test_mv_grid_extraction_with_real_av1() {
    ...
    let mv_grid = extract_mv_grid_from_parsed(&parsed)
        .expect("Failed to extract MV grid");
...
fn test_partition_grid_extraction_with_real_av1() {
    ...
    let partition_grid = extract_partition_grid_from_parsed(&parsed)
        .expect("Failed to extract partition grid");

    assert!(partition_grid.coded_width > 0, "Partition grid should have width");
    assert!(partition_grid.coded_height > 0, "Partition grid should have height");

    println!("Partition Grid: {}x{}, {} blocks",
        partition_grid.coded_width, partition_grid.coded_height, partition_grid.blocks.len()
    );

    // Verify all blocks are within frame bounds
    for block in &partition_grid.blocks {
        assert!(block.x < parsed.width(), "Block x {} should be < width {}", block.x, parsed.width());
        assert!(block.y < parsed.height(), "Block y {} should be < height {}", block.y, parsed.height());
    }
}
...
fn test_prediction_mode_extraction_with_real_av1() {
    ...
    // Count modes
    let intra_count = mode_grid.modes.iter()
        .filter(|m| m.map(|m| m.is_intra()).unwrap_or(false))
        .count();
    let inter_count = mode_grid.modes.iter()
        .filter(|m| m.map(|m| m.is_inter()).unwrap_or(false))
        .count();

    println!("Prediction Mode Grid: {}x{}, INTRA: {}, INTER: {}, Unknown: {}",
        mode_grid.grid_w, mode_grid.grid_h, intra_count, inter_count,
...
fn test_overlay_extraction_caching_works() {
    ...
    let base_qp = 128i16;
    let qp_grid1 = extract_qp_grid_from_parsed(&parsed, 0, base_qp)
        .expect("Failed to extract QP grid");

    // Second extraction - should use cache
    let qp_grid2 = extract_qp_grid_from_parsed(&parsed, 0, base_qp)
        .expect("Failed to extract QP grid");
