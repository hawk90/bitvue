                            ...
                            .pick_file()
                        {
                            ...
                            if let Ok(metadata) = std::fs::metadata(&path) {
                                ...
                                let ext = path.extension()
                                    .and_then(|e| e.to_str())
                                    .unwrap_or("unknown");
                                tracing::info!("File dialog: Selected {} file ({:.2} MB): {:?}",
                                    ext, size_mb, path);
                            } else {
            ...
            if !self.recent_files.is_empty() {
                ...
                for (i, path) in self.recent_files.iter().enumerate() {
                    ...
                    if ui.button(label).on_hover_text(path.display().to_string()).clicked() {
                        ...
                        if let Ok(metadata) = std::fs::metadata(path) {
                            let size_mb = metadata.len() as f64 / (1024.0 * 1024.0);
                            tracing::info!("File menu: Opening recent file ({:.2} MB): {:?}", size_mb, path);
                        } else {
    ...
    fn handle_yuvdiff_menu(&mut self, ui: &mut egui::Ui, _ctx: &egui::Context) {
        ui.menu_button("YUVDiff", |ui| {
                            ...
                            .pick_file()
                        {
                            ...
                            if let Ok(metadata) = std::fs::metadata(&path) {
                                ...
                                let ext = path.extension()
                                    .and_then(|e| e.to_str())
                                    .unwrap_or("unknown");
                                tracing::info!("YUVDiff: Loading {} file ({:.2} MB): {:?}",
                                    ext, size_mb, path);
                            } else {
            ...
            ui.menu_button("Subsampling", |ui| {
                if ui.radio_value(&mut self.yuv_diff_settings.subsampling, ChromaSubsampling::Yuv420, "Planar").clicked() {
                    tracing::info!("YUVDiff: Subsampling changed to YUV420 (Planar)");
                }
                if ui.radio_value(&mut self.yuv_diff_settings.subsampling, ChromaSubsampling::Yuv422, "Interleaved").clicked() {
            ...
            if ui.radio_value(&mut self.yuv_diff_settings.bit_depth, BitDepth::Bit8, "Use stream bitdepth").clicked() {
                tracing::info!("YUVDiff: Bit depth changed to 8-bit (stream)");
            }
            if ui.radio_value(&mut self.yuv_diff_settings.bit_depth, BitDepth::Bit10, "Use max stream bitdepth").clicked() {
                tracing::info!("YUVDiff: Bit depth changed to 10-bit (max stream)");
            }
            if ui.radio_value(&mut self.yuv_diff_settings.bit_depth, BitDepth::Bit12, "Use 16 bit bitdepth").clicked() {
    ...
    fn handle_options_menu(&mut self, ui: &mut egui::Ui, ctx: &egui::Context) {
        ui.menu_button("Options", |ui| {
            ...
            ui.menu_button("Color Space", |ui| {
                if ui.radio_value(&mut self.app_settings.color_space, ColorSpace::Bt601, "ITU Rec. 601").clicked() {
                    tracing::info!("Options: Color space changed to BT.601");
                }
                if ui.radio_value(&mut self.app_settings.color_space, ColorSpace::Bt709, "ITU Rec. 709").clicked() {
                    tracing::info!("Options: Color space changed to BT.709");
                }
                if ui.radio_value(&mut self.app_settings.color_space, ColorSpace::Bt2020, "ITU Rec. 2020").clicked() {
            ...
            ui.menu_button("CPU & Performance", |ui| {
                ...
                if ui.checkbox(&mut cpu_opt, "Enable CPU optimizations [avx2]").changed() {
            ...
            ui.menu_button("Theme & Layout", |ui| {
                ...
                ui.checkbox(&mut self.app_settings.auto_save_layout, "  Auto-save on exit")
    ...
    fn handle_mode_menu(&mut self, ui: &mut egui::Ui) {
        ui.menu_button("Mode", |ui| {
            egui::Grid::new("mode_menu_grid")
                ...
                .spacing([10.0, 4.0])
                .show(ui, |ui| {
                    if ui.button("Overview").on_hover_text("High-level statistics").clicked() {
                    ...
                    if ui.button("Coding Flow").on_hover_text("CTB grid with block types").clicked() {
                    ...
                    if ui.button("Prediction").on_hover_text("Intra/inter prediction modes").clicked() {
                    ...
                    if ui.button("Transform").on_hover_text("Transform unit tree").clicked() {
                    ...
                    if ui.button("QP Map").on_hover_text("Quantization parameter heatmap").clicked() {
            ...
            ui.menu_button("Data Export", |ui| {
                ...
                if let Some(ref units_model) = state.units {
                    ...
                    if let Some(path) = rfd::FileDialog::new()
                        ...
                        .save_file()
                    {
                        tracing::info!("Export: Frame sizes ({} units) to {:?}", unit_count, path);
            ...
            if ui.button("Unit Tree (JSON)").clicked() {
                ...
                if let Some(ref units_model) = state.units {
                    ...
                    if let Some(path) = rfd::FileDialog::new()
                        ...
                        .save_file()
                    {
                        tracing::info!("Export: Unit tree ({} units) to {:?}", unit_count, path);
                        self.set_success(format!("Exported to {}", path.display()));
                    } else {
                        tracing::debug!("Export dialog: Cancelled");
                    }
