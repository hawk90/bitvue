...
fn test_sps_various_level_idc() {
    ...
    for level in [10u8, 11u8, 12u8, 13u8, 20u8, 21u8, 22u8, 30u8, 31u8, 32u8, 40u8, 41u8, 42u8, 50u8, 51u8, 52u8] {
        let data = [
            0x42,  // profile_idc = 66
            0x00,  // constraint flags
            level,
...
fn test_sps_constraint_flags_combinations() {
    ...
    for flags in 0u8..=63u8 {
        let data = [
            ...
            0x1E,  // level_idc
            0x80, 0x01, 0x00, 0x80, 0x00, 0x80, 0x00, 0x80, 0x80, 0x80,
        ];
...
fn test_sps_various_dimensions() {
    for (width_mbs, height_mbs) in [(0u8, 0u8), (1, 1), (3, 3), (10, 10), (44, 44)] {
        let data = [
            0x42, 0x00, 0x1E,
            0x80, 0x01, 0x00, 0x80, 0x00, 0x80, 0x00, 0x80, 0x80, 0x80, 0x00,
...
fn test_sps_various_seq_parameter_set_ids() {
    for sps_id in 0u8..=10u8 {
        let data = [
            ...
            0x01, 0x00, 0x80, 0x00, 0x80, 0x00, 0x80, 0x80, 0x80,
        ];
