...
//! Foundations AV1 PlayerOverlay Future Plugin Lock #003: Frame Identity Contract
//! Subtask: S.T0-2.AV1.Foundations.PlayerOverlay.impl.future_plugin_lock.003
...
#[derive(Debug, Clone)]
struct PlayerOverlayFrame {
    display_idx: u64,
    decode_idx: u64,
}
struct PlayerOverlayState {
    frames: Vec<PlayerOverlayFrame>,
}
impl PlayerOverlayState {
    fn new() -> Self {
        Self { frames: Vec::new() }
    }
    fn add_frame(&mut self, f: PlayerOverlayFrame) {
        self.frames.push(f);
    }
    fn get_frame(&self, display_idx: u64) -> Option<&PlayerOverlayFrame> {
...
fn test_frame_by_display_idx() {
    let mut state = PlayerOverlayState::new();
    state.add_frame(PlayerOverlayFrame {
        display_idx: 0,
        decode_idx: 0,
    });
    state.add_frame(PlayerOverlayFrame {
...
#[test]
fn test_empty_state() {
    assert!(PlayerOverlayState::new().frames.is_empty());
}
#[test]
