...
contextBridge.exposeInMainWorld("bitvue", {
  ...
  openStream: (stream, filePath) =>
    ipcRenderer.invoke("bitvue:openStream", stream, filePath),
  closeStream: (stream) => ipcRenderer.invoke("bitvue:closeStream", stream),
  selectFrame: (stream, frameIndex) =>
    ipcRenderer.invoke("bitvue:selectFrame", stream, frameIndex),
  getHexRange: (stream, offset, len) =>
  ...
  setDebugYuvOffset: (offset) =>
    ipcRenderer.invoke("bitvue:setDebugYuvOffset", offset),
  ...
  getFrameAnalysis: (frameIndex) =>
    ipcRenderer.invoke("bitvue:getFrameAnalysis", frameIndex),
  getAv1Features: (frameIndex) =>
    ipcRenderer.invoke("bitvue:getAv1Features", frameIndex),
  ...
  exportEvidenceBundle: (params) =>
    ipcRenderer.invoke("bitvue:exportEvidenceBundle", params),
  // Returns a `data:image/png;base64,...` string, or null if there's no window to capture.
  captureScreenshot: () => ipcRenderer.invoke("bitvue:captureScreenshot"),
  ...
  getFrameSyntax: (stream, frameIndex) =>
    ipcRenderer.invoke("bitvue:getFrameSyntax", stream, frameIndex),
  ...
  onSidecarRestarted: (callback) => {
    ...
    return () =>
      ipcRenderer.removeListener("bitvue:sidecar-restarted", listener);
  },
});
