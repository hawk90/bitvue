...
describe('QualityMetricsPanel', () => {
  ...
  it('renders file selection buttons', () => {
    ...
    expect(screen.getByText(/Reference File \(Original\)/i)).toBeInTheDocument();
  ...
  it('disables calculate button when no files selected', () => {
    ...
    const button = screen.getByRole('button', { name: /calculate quality metrics/i });
  ...
  it('shows supported format info', () => {
    ...
    expect(screen.getByText(/Supported formats: IVF, MP4, MKV, WebM, YUV, Y4M/i)).toBeInTheDocument();
  });
