...
describe('DpbViewTab', () => {
  ...
  it('should render empty state icon when no current frame', () => {
    const { container } = render(<DpbViewTab currentFrame={null} frames={mockFrames} />);
  ...
  it('should render database icon', () => {
    const { container } = render(<DpbViewTab currentFrame={mockCurrentFrame} frames={mockFrames} />);
  ...
  it('should handle missing reference frames', () => {
    ...
    const { container } = render(<DpbViewTab currentFrame={frameWithInvalidRefs} frames={mockFrames} />);
  ...
  it('should render empty row message when no reference frames', () => {
    ...
    expect(screen.getByText('No reference frames (keyframe or intra-only)')).toBeInTheDocument();
  });

  it('should handle undefined ref_frames', () => {
    const frameWithUndefinedRefs = { ...mockCurrentFrame, ref_frames: undefined };
    render(<DpbViewTab currentFrame={frameWithUndefinedRefs} frames={mockFrames} />);
  ...
  it('should render note about DPB contents', () => {
    ...
    expect(screen.getByText('DPB shows current frame and its reference frames')).toBeInTheDocument();
  });
  ...
  it('should handle different frame types', () => {
    ...
    const bFrame = { ...mockCurrentFrame, frame_type: 'B', ref_frames: [10, 20] };
  ...
  it('should handle multiple reference frames', () => {
    const frameWithManyRefs = { ...mockCurrentFrame, ref_frames: [5, 10, 20, 30] };
    const { container } = render(<DpbViewTab currentFrame={frameWithManyRefs} frames={mockFrames} />);
