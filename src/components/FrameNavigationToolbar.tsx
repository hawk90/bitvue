...
function searchReducer(state: SearchState, action: SearchAction): SearchState {
  switch (action.type) {
    ...
    case 'SET_RESULTS':
      return { ...state, results: action.payload, selectedIndex: Math.min(state.selectedIndex, action.payload.length - 1) };
    case 'CLEAR':
      return { ...state, query: '', results: [], selectedIndex: 0 };
    case 'SELECT_PREV':
      return { ...state, selectedIndex: Math.max(0, state.selectedIndex - 1) };
    case 'SELECT_NEXT':
      return { ...state, selectedIndex: Math.min(state.results.length - 1, state.selectedIndex + 1) };
    default:
      return state;
  }
}
    ...
    <div className="frame-nav-toolbar">
      ...
      <div className="frame-nav-group">
        <button
          ...
          disabled={findPrevFrameByType(frames, currentFrameIndex, 'I') === null}
        ...
        <button
          ...
          disabled={findNextFrameByType(frames, currentFrameIndex, 'I') === null}
          title="Next I-Frame"
        >
          <span className="codicon codicon-arrow-right"></span>
          <span className="frame-nav-type-badge frame-type-i">I</span>
      ...
      {searchState.open && (
        <div className="frame-nav-search-dropdown">
            ...
            <div className="frame-nav-search-results">
              ...
              {searchState.results.slice(0, 10).map((idx, i) => {
                ...
                return (
                  ...
                  >
                    <span className={`frame-nav-result-badge frame-type-${frame.frame_type.toLowerCase()}`}>
                      {frame.frame_type}
                    </span>
                    <span className="frame-nav-result-index">#{frame.frame_index}</span>
                  </div>
                );
              })}
          ...
          {searchState.query && searchState.results.length === 0 && (
            <div className="frame-nav-search-empty">
              No results found
            </div>
          )}

          <div className="frame-nav-search-tips">
            <div className="frame-nav-search-tip">Type frame number or type. Use ↑↓ to navigate, Enter to select.</div>
          </div>
        </div>
      )}
    </div>
