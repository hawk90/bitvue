...
const mockFrames: FrameInfo[] = [
  ...
  { frame_index: 3, frame_type: 'P', size: 35000, poc: 3, temporal_id: 0, ref_frames: [1], display_order: 3, coding_order: 2 },
  { frame_index: 4, frame_type: 'I', size: 60000, poc: 4, temporal_id: 0, key_frame: true },
];
...
describe('ThumbnailsView', () => {
  ...
  it('should show loading state for loading thumbnails', () => {
    ...
    const loadingPlaceholders = document.querySelectorAll('.frame-placeholder.loading');
  ...
  it('should display thumbnail image when available', () => {
    ...
    expect(thumbnail).toHaveAttribute('src', 'data:image/png;base64,mockdata');
  });

  it('should mark current frame as selected', () => {
    render(<ThumbnailsView {...defaultProps} />);

    const selectedFrame = document.querySelector('[data-frame-index="0"].selected');
    expect(selectedFrame).toBeInTheDocument();

    const unselectedFrame = document.querySelector('[data-frame-index="1"].selected');
  ...
  it('should mark referenced frames', () => {
    ...
    const referencedFrame = document.querySelector('[data-frame-index="0"].is-referenced');
  ...
  it('should expand reference badge when clicked', () => {
    ...
    expect(defaultProps.onToggleReferenceExpansion).toHaveBeenCalledWith(1, expect.any(Object));
  });
  ...
  it('should apply correct frame type color class', () => {
    const getFrameTypeColorClass = vi.fn((type: string) => `frame-type-${type.toLowerCase()}`);
  ...
  it('should have proper ARIA attributes', () => {
    ...
    const frame = document.querySelector('[role="listitem"][data-frame-index="0"]');
  ...
  it('should set correct tabIndex for non-selected frames', () => {
    ...
    const frame = document.querySelector('[role="listitem"][data-frame-index="1"]');
...
describe('ThumbnailsView zoom', () => {
  it('should apply zoom transform', () => {
    ...
    const thumbnailsContainer = document.querySelector('.filmstrip-thumbnails');
...
describe('ThumbnailsView reference arrows', () => {
  ...
  it('should render SVG overlay when arrows exist', () => {
    const { usePreRenderedArrows } = require('@/components/Filmstrip/usePreRenderedArrows');
    usePreRenderedArrows.mockReturnValue({
      allArrowData: [{ sourceFrameIndex: 1, targetFrameIndex: 0, pathData: 'M0,0 L10,10' }],
      svgWidth: 1000,
    });
...
describe('ThumbnailsView edge cases', () => {
  ...
  it('should handle frames with only display_order', () => {
    const framesWithDisplayOrder = [
      { frame_index: 0, frame_type: 'I', size: 50000, poc: 0, display_order: 5 },
    ] as FrameInfo[];
  ...
  it('should handle frames with only coding_order', () => {
    const framesWithCodingOrder = [
      { frame_index: 0, frame_type: 'I', size: 50000, poc: 0, coding_order: 3 },
    ] as FrameInfo[];
  ...
  it('should handle all thumbnails loaded', () => {
    const allThumbnails = new Map(
      mockFrames.map(f => [f.frame_index, `data:image/png;base64,frame${f.frame_index}`])
    );
...
describe('ThumbnailsView React.memo', () => {
  it('should use React.memo for performance', () => {
    ...
    const initialContainer = document.querySelector('.filmstrip-thumbnails-container');

    rerender(<ThumbnailsView {...defaultProps} />);

    const rerenderedContainer = document.querySelector('.filmstrip-thumbnails-container');
  ...
  it('should re-render when currentFrameIndex changes', () => {
    ...
    const selectedFrame = document.querySelector('[data-frame-index="2"].selected');
    expect(selectedFrame).toBeInTheDocument();
  });
});
