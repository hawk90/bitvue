...
export type DisplayView = 'thumbnails' | 'sizes' | 'bpyramid' | 'timeline' | 'hrdbuffer' | 'enhanced';
...
export const FilmstripDropdown = memo(function FilmstripDropdown({ displayView, onViewChange }: FilmstripDropdownProps) {
  ...
  const handleSelectView = useCallback((view: DisplayView) => {
    onViewChange(view);
    setDropdownOpen(false);
  }, [onViewChange]);
  ...
  const getViewLabel = (view: DisplayView): string => {
    switch (view) {
      ...
      case 'sizes': return 'Frame Sizes';
      case 'bpyramid': return 'B-Pyramid';
      case 'timeline': return 'Timeline';
      case 'hrdbuffer': return 'HRD Buffer';
  ...
  return (
    <div ref={dropdownRef} className="filmstrip-dropdown">
      ...
      >
        ...
        <span className="codicon codicon-chevron-down" aria-hidden="true"></span>
      </button>
