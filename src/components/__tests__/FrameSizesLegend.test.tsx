...
describe('FrameSizesLegend', () => {
  ...
  it('should not apply active class to disabled metrics', () => {
    ...
    const inactiveItems = container.querySelectorAll('.legend-item:not(.active)');
  ...
  it('should start with default position', () => {
    ...
    const legend = container.querySelector('.frame-sizes-legend') as HTMLElement;
  ...
  it('should clamp position within window bounds', () => {
    ...
    if (handle) {
      ...
      const mouseMoveEvent = new MouseEvent('mousemove', { clientX: -1000, clientY: -1000 });
      document.dispatchEvent(mouseMoveEvent);
  ...
  it('should render all 8 metrics with correct colors', () => {
    ...
    indicators.forEach((indicator, i) => {
      expect((indicator as HTMLElement).style.backgroundColor).toBe(expectedColors[i]);
    });
  });
