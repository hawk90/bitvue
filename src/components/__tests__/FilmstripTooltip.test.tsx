...
describe('FilmstripTooltip', () => {
  ...
  it('should render frame size in KB', () => {
    render(<FilmstripTooltip frame={mockFrame} x={100} y={100} placement="right" />);
  ...
  it('should not render PTS when not available', () => {
    ...
    const { container } = render(
      <FilmstripTooltip frame={frameWithoutPts} x={100} y={100} placement="right" />
    );

    const ptsLabels = container.querySelectorAll('.filmstrip-tooltip-label');
    const hasPtsLabel = Array.from(ptsLabels).some(el => el.textContent === 'PTS');
  ...
  it('should not render POC when not available', () => {
    ...
    const { container } = render(
      <FilmstripTooltip frame={frameWithoutPoc} x={100} y={100} placement="right" />
    );

    const pocLabels = container.querySelectorAll('.filmstrip-tooltip-label');
    const hasPocLabel = Array.from(pocLabels).some(el => el.textContent === 'POC');
  ...
  it('should not render temporal ID when not available', () => {
    ...
    const { container } = render(
      <FilmstripTooltip frame={frameWithoutTemporal} x={100} y={100} placement="right" />
    );

    const temporalLabels = container.querySelectorAll('.filmstrip-tooltip-label');
    const hasTemporalLabel = Array.from(temporalLabels).some(el => el.textContent === 'Temporal');
  ...
  it('should not render reference frames when empty', () => {
    ...
    const { container } = render(
      <FilmstripTooltip frame={frameWithoutRefs} x={100} y={100} placement="right" />
    );

    const refLabels = container.querySelectorAll('.filmstrip-tooltip-label');
    const hasRefLabel = Array.from(refLabels).some(el => el.textContent === 'References');
  ...
  it('should position tooltip correctly', () => {
    ...
    const tooltip = container.querySelector('.filmstrip-tooltip') as HTMLElement;
