...
export const DebugPanel = memo(function DebugPanel({ frames, visible, onClose }: DebugPanelProps) {
  ...
  const stats = {
    total: frames.length,
    keyFrames: frames.filter(f => f.frame_type === 'KEY' || f.frame_type === 'I').length,
    pFrames: frames.filter(f => f.frame_type === 'P' || f.frame_type === 'INTER').length,
    bFrames: frames.filter(f => f.frame_type === 'B').length,
    withRefs: frames.filter(f => f.ref_frames && f.ref_frames.length > 0).length,
  };
  ...
  return (
    <>
      <button className="debug-toggle open" onClick={onClose} title="Close Debug Panel">
      ...
      <div className="debug-panel">
        ...
        <div className="debug-panel-content">
          ...
          {currentFrame && (
            <div className="debug-section">
              <div className="debug-section-title">Current Frame #{currentFrameIndex}</div>
              <div className="debug-info-row">
                <span className="debug-info-label">Frame Type</span>
                <span className="debug-info-value string">{currentFrame.frame_type}</span>
              </div>
              <div className="debug-info-row">
                <span className="debug-info-label">Size</span>
                <span className="debug-info-value number">{(currentFrame.size / 1024).toFixed(2)} KB</span>
              </div>
              <div className="debug-info-row">
                <span className="debug-info-label">Key Frame</span>
                <span className="debug-info-value boolean">{String(currentFrame.key_frame ?? false)}</span>
              </div>
              <div className="debug-info-row">
                <span className="debug-info-label">Display Order</span>
                <span className="debug-info-value number">{currentFrame.display_order ?? 'null'}</span>
              </div>
              <div className="debug-info-row">
                <span className="debug-info-label">Coding Order</span>
                <span className="debug-info-value number">{currentFrame.coding_order ?? 'null'}</span>
              </div>
              <div className="debug-info-row">
                <span className="debug-info-label">POC</span>
                <span className="debug-info-value number">{currentFrame.poc ?? 'null'}</span>
              </div>
              <div className="debug-info-row">
                <span className="debug-info-label">Ref Frames</span>
                <span className="debug-info-value">
                  {currentFrame.ref_frames ? (
                    <span className="debug-info-value string">[{currentFrame.ref_frames.join(', ')}]</span>
                  ) : (
              ...
              <div className="debug-info-row">
                ...
                <span className="debug-info-value">
                  {currentFrame.ref_slots ? (
                    <span className="debug-info-value string">[{currentFrame.ref_slots.join(', ')}]</span>
                  ) : (
                    <span className="debug-info-value null">null</span>
                  )}
                </span>
              ...
              {frames.slice(0, 50).map((frame) => (
                ...
                >
                  <div className="debug-frame-header">
                    <span className="debug-frame-index">#{frame.frame_index}</span>
                  ...
                  {frame.ref_frames && frame.ref_frames.length > 0 && (
                    <div className="debug-frame-details">
                      ...
                      <span className="debug-frame-detail-value">{frame.ref_frames.join(', ')}</span>
                    </div>
                  )}
                  <div className="debug-frame-details">
                    <span className="debug-frame-detail-label">Size:</span>
                    <span className="debug-frame-detail-value">{(frame.size / 1024).toFixed(1)}K</span>
                  </div>
                </div>
              ))}
            </div>
...
export const DebugPanelToggle = memo(function DebugPanelToggle({ isOpen, onToggle }: { isOpen: boolean; onToggle: () => void }) {
  return (
    <button
    ...
    >
      <span className={`codicon codicon-chevron-${isOpen ? 'right' : 'left'}`} />
    </button>
  );
});
