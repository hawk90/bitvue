...
if (!(window as any)[CONSOLE_OVERRIDE_KEY]) {
  ...
  console.log = (...args) => { tauriLog.log(String(args[0]), ...args.slice(1)); originalLog(...args); };
  console.info = (...args) => { tauriLog.info(String(args[0]), ...args.slice(1)); originalInfo(...args); };
  console.warn = (...args) => { tauriLog.warn(String(args[0]), ...args.slice(1)); originalWarn(...args); };
  console.error = (...args) => { tauriLog.error(String(args[0]), ...args.slice(1)); originalError(...args); };
  console.debug = (...args) => { tauriLog.debug(String(args[0]), ...args.slice(1)); originalDebug(...args); };

  originalLog('[main] Console override installed (once)');

  // Global error handlers to catch all unhandled errors
  window.addEventListener('error', (event) => {
    originalError('[GLOBAL ERROR]', event.message, event.filename, event.lineno, event.colno, event.error);
  });
