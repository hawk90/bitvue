  ...
  const toggleComponent = useCallback((component: YuvComponent) => {
    setComponentMaskState(prev => {
      ...
      if (index > -1) {
        ...
        const ordered = ORDER.filter(c => components.includes(c) || c === component);
...
// Mode metadata
export const MODES: { key: VisualizationMode; label: string; shortcut: string; description: string }[] = [
  { key: 'overview', label: 'Overview', shortcut: 'F1', description: 'High-level stream overview' },
  { key: 'coding-flow', label: 'Coding Flow', shortcut: 'F2', description: 'Encoder/decoder pipeline view' },
  { key: 'prediction', label: 'Prediction', shortcut: 'F3', description: 'Intra/inter prediction modes' },
  { key: 'transform', label: 'Transform', shortcut: 'F4', description: 'Transform coefficients' },
  { key: 'qp-map', label: 'QP Map', shortcut: 'F5', description: 'Quantization parameter heatmap' },
  { key: 'mv-field', label: 'MV Field', shortcut: 'F6', description: 'Motion vector field visualization' },
  { key: 'reference', label: 'Reference Frames', shortcut: 'F7', description: 'Frame dependency graph' },
];
