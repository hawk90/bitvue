...
import { SelectionProvider, useSelection, useSelectionSubscribe } from '../SelectionContext';
import type {
  TemporalSelection,
  FrameKey,
  UnitKey,
...
describe('SelectionContext temporal selection', () => {
  ...
  it('should clear temporal selection', () => {
    ...
    act(() => {
      result.current.setTemporalSelection({ type: 'point', frameIndex: 5 }, 'timeline');
    });
...
describe('SelectionContext bitRange selection', () => {
  ...
  it('should allow updating bitRange without affecting other selections', () => {
    ...
    act(() => {
      result.current.setFrameSelection({ stream: 'A', frameIndex: 10 }, 'timeline');
    });

    act(() => {
      result.current.setBitRangeSelection({ startBit: 500, endBit: 1000 }, 'hex');
    });

    expect(result.current.selection?.frame).toEqual({ stream: 'A', frameIndex: 10 });
    expect(result.current.selection?.bitRange).toEqual({ startBit: 500, endBit: 1000 });
  });
});
...
describe('SelectionContext tri-sync rules', () => {
  ...
  it('should propagate temporal selection to frame selection (rule 1)', () => {
    ...
    act(() => {
      result.current.setTemporalSelection({ type: 'point', frameIndex: 42 }, 'timeline');
    });
  ...
  it('should not override existing frame selection when setting temporal', () => {
    ...
    act(() => {
      ...
      result.current.setTemporalSelection({ type: 'point', frameIndex: 30 }, 'timeline');
    });
...
describe('SelectionContext subscription system', () => {
  ...
  it('should unsubscribe callback when returned function is called', () => {
    ...
    act(() => {
      result.current.setFrameSelection({ stream: 'A', frameIndex: 20 }, 'timeline');
    });
  ...
  it('should support multiple subscribers', () => {
    ...
    act(() => {
      result.current.setFrameSelection({ stream: 'A', frameIndex: 5 }, 'keyboard');
    });
  ...
  it('should notify subscribers with source information', () => {
    ...
    act(() => {
      result.current.setTemporalSelection({ type: 'point', frameIndex: 15 }, 'main');
    });
...
describe('SelectionContext clearAll', () => {
  ...
  it('should notify subscribers on clear', () => {
    ...
    act(() => {
      result.current.setFrameSelection({ stream: 'A', frameIndex: 5 }, 'timeline');
      result.current.clearAll();
    });
...
describe('SelectionContext stream switching', () => {
  ...
  it('should switch from stream A to stream B', () => {
    ...
    act(() => {
      result.current.setFrameSelection({ stream: 'A', frameIndex: 10 }, 'timeline');
      result.current.setFrameSelection({ stream: 'B', frameIndex: 20 }, 'timeline');
    });
  ...
  it('should maintain selections when switching streams', () => {
    ...
    act(() => {
      result.current.setFrameSelection({ stream: 'A', frameIndex: 5 }, 'timeline');
      result.current.setBitRangeSelection({ startBit: 100, endBit: 200 }, 'hex');
    });

    const originalBitRange = result.current.selection?.bitRange;

    act(() => {
      result.current.setFrameSelection({ stream: 'B', frameIndex: 15 }, 'timeline');
    });
...
describe('SelectionContext complex workflows', () => {
  ...
  it('should handle syntax tree navigation workflow', () => {
    ...
    expect(result.current.selection?.bitRange).toEqual({ startBit: 500, endBit: 508 });

    // User then selects frame from filmstrip
    act(() => {
      result.current.setFrameSelection({ stream: 'A', frameIndex: 25 }, 'filmstrip');
    });

    expect(result.current.selection?.frame).toEqual({ stream: 'A', frameIndex: 25 });
    expect(result.current.selection?.temporal).toEqual({ type: 'point', frameIndex: 25 });
  });
  ...
  it('should handle hex view navigation workflow', () => {
    ...
    act(() => {
      result.current.setBitRangeSelection({ startBit: 8100, endBit: 8200 }, 'hex');
    });

    expect(result.current.selection?.bitRange).toEqual({ startBit: 8100, endBit: 8200 });
  ...
  it('should handle timeline scrubbing workflow', () => {
    ...
    for (let i = 0; i < 10; i++) {
      act(() => {
        result.current.setTemporalSelection({ type: 'point', frameIndex: i }, 'timeline');
      });
...
describe('SelectionContext edge cases', () => {
  ...
  it('should handle rapid selection changes', () => {
    ...
    act(() => {
      for (let i = 0; i < 50; i++) {
        result.current.setFrameSelection({ stream: 'A', frameIndex: i }, 'keyboard');
      }
    });
  ...
  it('should handle selection with all source types', () => {
    ...
    const sources: Array<'syntax' | 'hex' | 'main' | 'timeline' | 'filmstrip' | 'reference-lists' | 'keyboard' | 'minimap' | 'bookmarks'> = [
      'syntax',
      'hex',
      'main',
      'timeline',
    ...
    sources.forEach(source => {
      act(() => {
        result.current.setFrameSelection({ stream: 'A', frameIndex: 5 }, source);
      });
  ...
  it('should handle zero-sized bit ranges', () => {
    ...
    act(() => {
      result.current.setBitRangeSelection({ startBit: 100, endBit: 100 }, 'hex');
    });

    expect(result.current.selection?.bitRange).toEqual({ startBit: 100, endBit: 100 });
  });
  ...
  it('should handle very large frame indices', () => {
    ...
    act(() => {
      result.current.setFrameSelection({ stream: 'A', frameIndex: 999999 }, 'timeline');
    });
  ...
  it('should handle negative frame indices in temporal selection', () => {
    ...
    act(() => {
      result.current.setTemporalSelection({ type: 'point', frameIndex: -1 }, 'timeline');
    });
...
describe('SelectionContext field type size estimation', () => {
  ...
  it('should estimate correct size for u1 field', () => {
    ...
    expect(result.current.selection?.bitRange).toEqual({ startBit: 100, endBit: 101 });
  });
  ...
  it('should estimate correct size for u4 field', () => {
    ...
    expect(result.current.selection?.bitRange).toEqual({ startBit: 200, endBit: 204 });
  });
  ...
  it('should estimate correct size for u8 field', () => {
    ...
    expect(result.current.selection?.bitRange).toEqual({ startBit: 300, endBit: 308 });
  });
  ...
  it('should handle variable length fields', () => {
    ...
    expect(result.current.selection?.bitRange).toEqual({ startBit: 400, endBit: 400 });
  });
  ...
  it('should handle leb128 field type', () => {
    ...
    expect(result.current.selection?.bitRange).toEqual({ startBit: 500, endBit: 500 });
  });
});
