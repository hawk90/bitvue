...
describe('createMenuItem', () => {
  ...
  it('should create menu item with accelerator', async () => {
    ...
    const item = await createMenuItem({ id: 'test-id', text: 'Test', accelerator: 'Cmd+O' });
  ...
  it('should create menu item with event and detail', async () => {
    ...
    await createMenuItem({ id: 'test-id', text: 'Test', event: 'menu-click', eventDetail: detail });
...
describe('createSubmenu', () => {
  ...
  it('should handle submenu with many items', async () => {
    ...
    mockMenuItemNew.mockImplementation((config: any) => Promise.resolve(config));
    mockSubmenuNew.mockResolvedValue(mockSubmenu);

    const items = await Promise.all(
      Array.from({ length: 50 }, (_, i) => createMenuItem({ id: `item-${i}`, text: `Item ${i}` }))
    );
  ...
  it('should handle nested submenus', async () => {
    ...
    const innerItem = await createMenuItem({ id: 'inner-item', text: 'Inner Item' });
  ...
  it('should preserve item order', async () => {
    ...
    const items = await Promise.all(
      ['first', 'second', 'third'].map((id, i) => createMenuItem({ id, text: `Item ${i}` }))
    );
...
describe('Menu Integration', () => {
  ...
  it('should create complex menu structure', async () => {
    ...
    const fileMenu = await createSubmenu({
      text: 'File',
      items: [
        await createMenuItem({ id: 'new', text: 'New', accelerator: 'Cmd+N', event: 'file-new' }),
        await createMenuItem({ id: 'open', text: 'Open', accelerator: 'Cmd+O', event: 'file-open' }),
      ],
    });
