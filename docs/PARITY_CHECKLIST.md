...
## Layer 11: Panel-level visual parity (Timeline/Filmstrip vs VQ Analyzer, 2026-08-23)

First pass of a new panel-by-panel visual comparison against a real competitor screenshot source
(VQ Analyzer's own User Guide PDF, real UI screenshots, not marketing copy) rather than the
interaction-contract-only comparisons Layers 8-10 did. Timeline/Filmstrip was the first panel
covered; found via direct user interaction with the real running app, not just static screenshots
(see PNL-02's discovery).

| ID | Item | Severity | Status | Notes |
|----|------|----------|--------|-------|
| PNL-01 | Filmstrip/Timeline bar resizable against Main View | P2 | [x] | **Fixed 2026-08-23**: was a CSS-fixed-height `<div>` explicitly `"outside Group so height is CSS-controlled, not draggable"` (`DockableLayout.tsx`'s own prior comment — a deliberate original choice, not an oversight) while every other panel boundary in the layout uses `react-resizable-panels`. Moved into the vertical `Group` as a real `Panel` + `Separator`, new `PANEL_SIZES.FILMSTRIP_BAR`/`PANEL_MIN_SIZES.FILMSTRIP_BAR` constants (default 30%, min 12% — the real floor is `TimelineFilmstrip.css`'s own 120px `min-height`, intentionally under that so the CSS floor is what actually stops the drag). `TimelineFilmstrip.css`'s internal Timeline/Filmstrip split was already flex-based (`flex:1`/`min-height`), not the fixed-pixel split its own stale comment claimed — needed no change, just a comment fix. 3 new tests + full vitest (2370) + typecheck clean; verified end-to-end with a real Electron screenshot at both the default window size and the app's own documented 900x600 hard floor (new `BITVUE_ELECTRON_WINDOW_SIZE` harness hook) |
| PNL-02 | Reference-frame arrows in Thumbnails/Enhanced filmstrip views | P2 | [x] | **Found and fixed 2026-08-23** — a real, always-reproducible bug, found by the user clicking a thumbnail in the live app (not by a static screenshot, which only ever showed the default unselected state): clicking any frame with real `ref_frames` correctly highlighted its referenced frames' borders amber (`is-referenced`, already working), but the actual connecting arrow (`usePreRenderedArrows`' "down, across, up" ㄷ-shaped SVG path) rendered as an invisible 1px stub — `.filmstrip-thumbnails-container`/`.filmstrip-thumbnails` both clip vertical overflow (`overflow: hidden`/`auto hidden`) and the frame cards filled 100% of the container height with zero slack below them for the arrow's `verticalOffset = 30 + slotIndex*12`px drop. Fixed with a real `padding-bottom: 80px` reservation on `.filmstrip-thumbnails` (covers up to ~4 stacked reference slots) plus PNL-01's larger default panel height (30% vs the prior ~22%) so the reservation has real room to render into. Verified against a real AV1 compound-prediction frame with 4 active reference slots (REF0-REF3) via a real Electron screenshot — all 4 now draw a real connecting line to their target frame (previously 0 were visible beyond the 1px stub). **Label/line alignment fixed same day**, found via the user's own live interactive testing (clicking a 3rd reference-bearing frame themselves, not from a screenshot): `usePreRenderedArrows`' `labelY` used `verticalOffset / 2` while the line's own horizontal segment (drawn by the caller-supplied `calculatePath`) sits at the full `verticalOffset` -- every slot's label landed in one shared 6px-tall band, aligned with none of its own lines. Fixed by using the same `verticalOffset` for both, confirmed via a real Electron screenshot (REF0's label now sits exactly on REF0's own line) and 2 new hook-level tests (`tests/hooks/usePreRenderedArrows.test.tsx`, previously zero coverage for this calculation -- the existing `ThumbnailsView`/`VirtualizedThumbnailsView` tests only ever mock `usePreRenderedArrows` entirely) asserting the label's Y literally equals the parsed line path's turn-point Y, not just a hand-computed constant. Full vitest (2372) + typecheck clean |
| PNL-03 | `ref_frames` stored raw AV1 DPB slot numbers, not real frame indices | P1 | [x] | **Found and fixed 2026-08-23** — a real, deep data bug found while investigating PNL-02: `bitvue-indexer::index_ivf_av1` (`unit.ref_frames = Some(ref_idx.iter().map(\|&x\| x as usize).collect())`) copied AV1's `ref_frame_idx[]` (spec 5.9.2, `u(3)` — a DPB **slot** number 0-7, not a frame index) directly into `ref_frames` with only a type cast. Proof: clicking frame 40 then frame 80 in the live app (40 frames apart) showed the *exact same* `ref_frames` array `[0,4,0,0,2,5,5]` for both — impossible for real frame indices, expected for raw slot numbers cycling through only 8 possible values. Every reference arrow/highlight in Thumbnails/Enhanced (and likely B-Pyramid) had been pointing at essentially arbitrary low-numbered frames for any frame past the first ~8, coincidentally plausible-looking only because early frame indices and early slot numbers are both small. **Fixed properly, not patched**: switched `index_ivf_av1` from the deliberately-approximate `parse_frame_header_basic` (module doc: skips/guesses several fields upstream of `ref_frame_idx`'s bit position, so even a "correctly-coded" `u(3)` read there isn't reliably at the right bit offset) to the real spec-accurate `parse_frame_header_full` + `RefFrameState` — the same already-tested primitives 5 `bitvue-sidecar` command modules already rely on for this exact class of problem. Added a new indexer-local `slot_frame_index: [Option<usize>; 8]` tracker (deliberately NOT merged into the shared `RefFrameState`, which only tracks `order_hint` for a different need, to avoid touching its other 5 callers) updated via each frame's real `refresh_frame_flags` after computing that frame's own translated `ref_frames` (order matters: a frame's references reflect DPB state *before* it, its own refresh only affects frames *after* it). Falls back to the old basic-parser path (no `ref_frames`, not a guessed one) if no sequence header exists at all, or permanently once any full-header parse fails mid-stream (state can't be trusted past a desync) — an honest "we don't know" beats a specific-looking wrong answer either way. 3 new Rust tests against the real fixture (frame-40-vs-frame-80 must now differ; every `ref_frames` value must be `< frame_index`, real fixture; keyframe has none) + full `cargo test --workspace --exclude abseil --lib --tests` clean + `cargo fmt`/`clippy -D warnings` clean. Verified against the real running app: frame 69's hover tooltip now shows `References [68, 67, 66, 60, 0, 0, 61]` — realistic temporal-locality references (mostly very recent frames + the persistent frame-0 backup), not the old constant `[0,4,0,0,2,5,5]` |
| PNL-04 | Reference-arrow algorithm rewrite: dedup, distant-target visibility, coordinate math | P1 | [x] | **Found and fixed 2026-08-23**, same day as PNL-02/03 — user invoked deep review ("ultrathink") after PNL-02/03 landed, since arrows were still not rendering correctly for real multi-slot frames like frame 69 (`References [68,67,66,60,0,0,61]`, 7 raw slots). Four compounding issues, all in `usePreRenderedArrows.ts`/`VirtualizedThumbnailsView.tsx`: (1) **No dedup by target** — AV1 commonly points several slots at the same real frame (e.g. GOLDEN/ALTREF sharing a long-term slot, frame 69's two slots both `0`); drawing one arrow per raw slot drew fully overlapping duplicate lines. Fixed by grouping `refFrameIndices` by target frame first, one arrow per unique target with a combined label (`"REF4,REF5"`). (2) **Padding still undersized** — PNL-02's 80px covered ~4 slots; AV1's real max is 7. Re-provisioned to 130px padding-bottom / 34% default panel height (from 80px/30%). (3) **Distant references never mounted** — `VirtualizedThumbnailsView`'s window is `currentFrameIndex ± 50`; a reference like frame 69→0 falls outside it, so `usePreRenderedArrows`' `framePositions.get(refIdx)` silently returned undefined and the arrow was dropped with no explanation. Fixed with a capped window-widening (`MAX_EXTRA_WINDOW_FOR_REFS = 100`): `getVisibleRange` widens `start` (never `end`, since AV1 references always point backward) to cover the current frame's own nearest out-of-window reference, capped so a stream that keeps frame 0 as a permanent rolling reference can't force-mount the entire file. (4) **The real root cause, found only after (1)-(3) still didn't fully fix rendering**: `ArrowPosition` measurement used `getBoundingClientRect()` (viewport-relative), which silently breaks the moment a target is scrolled out of the currently-visible area within its scrollable container — confirmed via temporary debug instrumentation showing large, still-drifting negative coordinates even after 4000ms of extra settle time (ruling out a timing race). This was a pre-existing latent bug, only newly exposed by (3) pulling in genuinely far-scrolled mounted targets that were previously never simultaneously mounted *and* far-scrolled. Fixed by switching to `offsetLeft`/`offsetTop`/`offsetWidth`/`offsetHeight` (relative to the positioned `.filmstrip-thumbnails` container ancestor, scroll-position-independent). 3 hook-level tests updated to stub the new offset properties instead of `getBoundingClientRect`, all 12 arrow/virtualization tests + full vitest (2376) + typecheck clean. Verified against the real running app: frame 69 now renders a correctly-aligned `REF0` arrow+label with sane on-screen coordinates (previously either invisible or drawing at wildly wrong negative positions) |
| PNL-05 | Arrow label rendered as a strikethrough by its own (and other arrows') lines | P1 | [x] | **Found and fixed 2026-08-23**, later same day — user reported PNL-04 still visibly broken with a real screenshot: a merged label like `REF0,REF1,REF2,REF3,REF4,REF5` appeared cut in the middle, as if two separate labels. Root cause (found via 3x-zoomed crops of real screenshots, isolating a clean single-arrow case to rule out confounds): the label's `labelY` is deliberately equal to its own line's horizontal-segment Y (see PNL-02's alignment fix), so for a wide horizontal run the line passed directly through the bare glyphs' vertical center with nothing to occlude it -- a real rendering defect, not a data or geometry bug. First attempt (a `<rect>` background chip drawn between `path` and `text` in the same `<g>`) visibly did nothing: `.thumbnail-arrows-overlay path`'s existing `transform: translateZ(0)` (a forced-compositing rule, kept for reliable off-screen rendering) promotes every `<path>` onto its own layer, and CSS paints promoted elements in a separate, document-order-based phase -- a non-promoted `<rect>`/`<text>` after it in the DOM does NOT reliably paint on top regardless of source order. Promoting the label's own `<g>` the same way fixed a label against **its own** arrow's line, but a real screenshot of a second, crossing arrow (frame 2, `ref_frames=[0,0,0,0,0,0,1]`, two targets) showed the SECOND arrow's line still cutting through the FIRST arrow's already-fixed label -- because each arrow's path+label were interleaved per-arrow, so a later arrow's promoted path still out-ordered an earlier arrow's promoted label in the flattened document-order paint sequence. **Real fix**: restructured both `ThumbnailsView.tsx` and `VirtualizedThumbnailsView.tsx` from one `allArrowData.map()` emitting each arrow's path+label together into two separate passes over the same list -- all lines first, then all labels (each label's chip+text now promoted via a new `.thumbnail-arrows-overlay .arrow-label` CSS rule mirroring `path`'s) -- guaranteeing every label's document-order position is after every path's, system-wide, not just within its own arrow. Also had to switch the chip's `fill` from an SVG presentation attribute (`fill="var(--bg-app)"`) to `style={{fill: "var(--bg-app)"}}`; the presentation-attribute form silently failed to resolve the CSS custom property (root cause not further isolated, empirically confirmed via an interim `fill="magenta"` test that DID render, proving the promotion fix itself was already correct and only the color resolution was broken). 1 test selector updated (`.thumbnail-arrows-overlay > g > path` → `> path`, matching the flattened structure), full vitest (2376) + typecheck clean. Verified via real Electron screenshots at 3x zoom on both the single-arrow case (frame 1) and the crossing-arrow case (frame 2) -- labels now render fully legible with lines visibly stopping at the label's edge instead of passing through |
| PNL-06 | Filmstrip arrow-area height was a hardcoded worst-case constant | P2 | [x] | **Fixed 2026-08-23**, alongside PNL-05 in response to the user's explicit instruction to "calculate frame and line geometry together and adjust the height/viewport accordingly" rather than continuing to hand-tune magic-number padding (PNL-02/PNL-04 had already iterated 50px→80px→130px). `usePreRenderedArrows` now tracks `maxArrowOrdinal` while building each frame's deduped arrow stack and returns `maxStackDepth` (the deepest stack across the whole mounted window, not just the selected frame, so switching between two already-mounted frames never causes a layout jump). Both views compute `arrowAreaHeight = ARROW_BASE_OFFSET + (maxStackDepth-1)*ARROW_SPACING_PER_SLOT + 20` and apply it as an inline `paddingBottom` (always overrides the CSS rule, which is now `padding-bottom: 0` with a comment explaining why). A simple stream with only 1-2 reference slots now reserves proportionally less empty space instead of always reserving room for AV1's 7-slot worst case; a real 7-slot frame still computes ~114px, close to the old hardcoded 130px, confirming the calculation is sound at the boundary. `ARROW_BASE_OFFSET`/`ARROW_SPACING_PER_SLOT` constants deduplicated into module-level values shared between the height calculation and `calculateThumbnailPath`, so the reserved space and the actual line positions can never drift apart. |
| PNL-07 | Deep reference-arrow stacks (5-7 slots) still invisible/clipped despite PNL-06's correct dynamic padding | P1 | [x] | **Found and fixed 2026-08-23**, same day, after the user reported PNL-05/06 still broken with a real screenshot and demanded a holistic ("overall"/"거시적") fix rather than another local patch. Root cause was NOT in the arrow-rendering code at all (PNL-02/04/05/06 were all real, correct fixes) but in a same-day-earlier, unrelated structural change: PNL-01 moved `.filmstrip` into a real `react-resizable-panels` `Panel` (`DockableLayout.tsx`'s `filmstrip-bar`). That library hardcodes `overflow: hidden` on the Panel's own wrapper div with **no prop to override it** ("Prevent Panel content from interfering with panel size" — verified by reading `node_modules/react-resizable-panels/dist/react-resizable-panels.js`'s `Panel` source directly). Meanwhile `Filmstrip.css`'s `.filmstrip` rule still carried its pre-Panel sizing (`height: auto; min-height: 140px; max-height: var(--filmstrip-height, 180px)` — the var resolves to a static 140px, `theme/colors.css`; `overflow: visible`), a leftover from when `.filmstrip` sat in "a fixed (non-resizable-Panel) flex slot" per its own now-stale comment. The two combined into an unfixable clip: `.filmstrip` self-capped at 140px regardless of how much room the (often larger, user-resizable) Panel actually granted, `overflow: visible` let a deep stack's real content spill past that 140px cap, and the spillover then hit the Panel's own hardcoded `overflow: hidden` one level up — with **no scrollbar possible at that boundary**, since neither `.filmstrip` (`visible`) nor the Panel (`hidden`, not `auto`) ever create one. This is why an interim fix (`.filmstrip-thumbnails-container`'s `overflow: hidden` → `hidden auto`, confirmed via computed-style debugging to be genuinely configured correctly with real overflow: `overflowY: "auto"`, `scrollHeight: 190 > clientHeight: 116`) never produced a visible scrollbar in any screenshot — it was several DOM layers below the actual, un-stylable clip point, so its own correctly-configured scrollbar never got a chance to render. **Real fix**: `.filmstrip` now just fills whatever box the Panel gives it (`height: 100%; overflow: hidden;`, dropping the vestigial min/max-height cap entirely — sizing is the Panel's job now, not this file's), and `.filmstrip-content` (the flex:1 child directly inside it) became the single scroll authority (`overflow: auto` unconditionally, replacing the old `auto hidden` + 3 duplicate per-view-mode `overflow-y: auto` overrides for bpyramid/enhanced/hrdbuffer, now redundant and removed; also removed a `.filmstrip-content.showing-reference-lines { overflow-y: visible }` escape hatch that was actively wrong post-Panel-wrapping for the same reason). Net effect: `.filmstrip` now actually uses the Panel's full (usually much larger than 140px) allocated height, so realistic reference-stack depths fit with zero scrolling in practice, with `.filmstrip-content`'s scroll as a genuine safety net for pathological cases that still don't fit. Verified via real Electron screenshots of frame 6 (`maxStackDepth=6`, all of REF0-REF6 fully visible, zero clipping) on **both** `ThumbnailsView` and `VirtualizedThumbnailsView` (temporarily raised `VIRTUALIZATION_THRESHOLD` to force the non-virtualized path against the same 250-frame fixture, confirmed identical, then reverted) — both the "arrows disappear going downward" and "lines overlap when they exit" complaints resolved (each stacked line/label renders at a clean, non-overlapping vertical offset with no strikethrough). Full vitest (2376 tests, 118 files) + typecheck + `npm run build` clean. |
| PNL-08 | Reference-arrow takeoff point was a single shared pixel ("one trunk"); backward/forward refs shared one stacking tower | P2 | [x] | **Found and fixed 2026-08-24**, next day, after the user visually flagged both issues directly ("레퍼런스 선이 하나의 줄기에서 나오잖아... 적당한 거리 알고리즘으로 나누면 안됨? 그리고 너무 층층층이 쌓지 않아? 오른쪽이나 왼쪽으로 갈때?" — "the reference lines all come out of one single stem, can't we split that with a distance algorithm? And doesn't it stack too many layers when going right or left?"). Two independent fixes, both in `usePreRenderedArrows.ts` (shared by `ThumbnailsView`/`VirtualizedThumbnailsView`/`BPyramidTimeline`): (1) **Single-trunk takeoff**: every one of a frame's stacked ㄷ-shaped arrows (`calculateThumbnailPath`/`calculateBelowPath`) dropped straight down from the exact same `sourcePos.centerX` pixel before turning horizontal — visually one thick trunk that only separated into a comb of lines well below the frame (confirmed via a real screenshot of frame 6's 5-deep stack). Fixed by fanning the takeoff X out per stacking ordinal (`fanSpacing = 6px * ordinal`, leaning toward whichever side the line is about to turn), duplicated identically in the hook (for label `sourceX`) and each ㄷ-shaped path calculator (for the line itself) — same "must match" duplication pattern PNL-02/06 already established for `verticalOffset`. (2) **Shared stacking tower**: the stacking ordinal was one counter shared across all of a frame's refs regardless of direction, so a frame with e.g. 4 backward + 3 forward refs (a B-frame referencing a future frame) reserved a 7-deep tower when neither direction is actually more than 4 deep — backward (`target < source`) and forward (`target > source`) lines diverge in opposite screen directions the instant they leave the source, so they never visually compete for vertical room and don't need to share one stack. Fixed by tracking `backwardOrdinal`/`forwardOrdinal` as two independent counters, with `maxStackDepth = max(maxBackward, maxForward) + 1` (not their sum). `av1_test.ivf` (the local fixture) is P-only, so the forward-direction split couldn't be verified against a real screenshot — verified instead with a new synthetic unit test (`usePreRenderedArrows.test.tsx`, 4th test) proving 2 backward + 1 forward ref get `slotIndex` 0/1/0 (not 0/1/2) and `maxStackDepth` 2 (not 3). Fan-out itself verified via a real Electron screenshot of frame 6 (5 unique targets, all backward in this fixture) — the takeoff lines now visibly spread across ~24px instead of sharing one pixel. Full vitest (2377 tests, 118 files) + typecheck + `npm run build` clean. |
| PNL-09 | Frame Sizes bar chart's tallest bar/line touched the container's top edge with zero headroom | P3 | [x] | **Found and fixed 2026-08-24**, same day, after the user asked whether the Frame Sizes tab matched the oracle and flagged the left/right extremes as looking wrong ("좌우 극단에 너무 이상하지 않음?"). Cross-checked `FrameSizesView.tsx`'s bar heights against `bitvue-cli decode --stats` on `av1_test.ivf` end-to-end — every bar height (including the apparent "spikes" near the right edge, frames 225/231) matched real per-frame byte sizes exactly; this fixture's periodic large-P-frame pattern repeats throughout the whole stream, not just at the edges, so there was no left/right-specific data or scaling bug. The one real, confirmed issue (via a real screenshot, zoomed on frame 0's bar): `barHeight = (frame.size / maxFrameSize) * 100` means the single largest frame in the stream — almost always frame 0, the I-frame, i.e. always the left extreme in practice — renders at exactly 100% height and sits flush against `.frame-sizes-bars`' top edge, reading as "cut off," while the "10 KB" axis label above it has visible padding and floats clearly below the true top, making the two look misaligned. Fixed by introducing `Y_AXIS_HEADROOM = 1.08` and scaling every bar/line/metric-line height or position (`barHeight`, the bitrate-curve and moving-avg SVG polylines, and the avg/min/max metric lines) against `maxFrameSize * Y_AXIS_HEADROOM` / `maxBitrate * Y_AXIS_HEADROOM` instead of the raw max — the axis labels deliberately keep showing the true unscaled max/mid/min, since headroom is a rendering margin, not a change to what the numbers mean. 2 existing test assertions (`FrameSizesView.test.tsx`) updated to the new headroom-adjusted percentages. Verified via a before/after real Electron screenshot pair: the tallest bar now visibly stops short of the container's top edge (rounded corner is visible above it) instead of touching it. Separately flagged, not fixed, since it's a product/scale-design decision rather than a bug: because I-frames are typically far larger than P/B-frames and the chart uses one global linear max, the I-frame's bar dominates the scale and visually crushes all P/B-frame variation into thin slivers near the bottom — real analyzer tools often exclude I-frames from the auto-scale or use a percentile/log scale for exactly this reason; needs a product decision, not a silent change. Full vitest (2377 tests, 118 files) + typecheck + `npm run build` clean. |
| PNL-10 | Frame Sizes chart's left (Bitrate) and right (QP) Y-axis labels didn't line up with each other or with the bar/line plot area | P2 | [x] | **Found and fixed 2026-08-24**, right after PNL-09, when the user clarified their original "extremes look wrong" report actually meant the Y-axis itself, not the bar data ("와이 축이 지금 정렬이 안되어있잖아 좌측이랑 우측이" — the left and right Y-axis aren't aligned). Root cause: `.frame-sizes-axis` (both `axis-left` and `axis-right`) was a 4-item `justify-content: space-between` flex column — `max`/`mid`/`min` labels *plus* `.axis-title` ("Bitrate"/"QP") sharing the same distribution — while `.frame-sizes-chart` (the actual bar/line plot area) had no equivalent title item eating into its own height. So the axis's "50%"/"0%" landed wherever 4-way space-between put them, not at the plot area's actual 50%/0% — identically wrong on both sides (same class, same structure), which is why it read as "both left and right," not a left-vs-right mismatch. Fixed by restructuring both the axis and the chart column into a matching 2-part split: axis → `.axis-scale` (the 3 labels, `flex: 1`) + `.axis-title` (fixed `height: 16px`); chart → `.frame-sizes-plot` (bars/lines/metric-lines, `flex: 1`) + `.frame-sizes-chart-footer` (an empty spacer, same fixed `height: 16px`). With all three columns' "flex: 1 scale box" now the exact same height, `.axis-label`s switched from space-between to absolute positioning at literal `top: 0` / `top: 50%` / `bottom: 0` within `.axis-scale` — guaranteed exact alignment with the plot area's own 0%/50%/100%, immune to text/font-metric differences. Also removed a dead/contradictory `.axis-left .axis-label { text-align: right }` / `.axis-right .axis-label { text-align: left }` override pair that would have fought the new per-label (max/mid/min) text alignment. Verified via a real Electron screenshot, zoomed on the bar-chart row: "10 KB"/"51" (both `max`) now sit on the exact same horizontal line, as do "5 KB"/"26" (both `mid`). Full vitest (2377 tests, 118 files, all 43 `FrameSizesView` tests unchanged and still passing) + typecheck + `npm run build` clean. **Self-correction, same session**: the user immediately reported "still not aligned" after this fix was described. Re-verification found the vertical label alignment itself was actually correct (re-confirmed via a fresh, more precise zoomed screenshot) -- the real problem was a regression *introduced by this same fix*: rewriting `.frame-sizes-chart` from a row to a column dropped its pre-existing `flex: 1` (the property that fills the remaining row width between the two axis columns in `.filmstrip-sizes`, an axis totally unrelated to the new `flex-direction: column`), collapsing the whole chart to its min-content width -- visually a much bigger, more obvious break (chart squeezed into roughly the left third of its old width, big dead space before the right axis) than the sub-pixel label issue this fix was meant to solve. Restored `flex: 1` alongside `flex-direction: column` (the two are independent axes, no conflict). Re-verified via a fresh screenshot: chart fills the full width again, both label rows still aligned. Full vitest (2377, 118 files) + typecheck + `npm run build` clean again. **Second self-correction, same session**: the user reported "still misaligned" a second time even after the width regression fix; a precise pixel-measured re-check confirmed the two axes really were aligned with *each other* (sub-1px), so the remaining gap had to be axis-vs-data. Found it: `.axis-label`s were positioned at literal `top: 0` / `top: 50%` / `bottom: 0`, but bars/lines in `.frame-sizes-plot` scale against `maxFrameSize * Y_AXIS_HEADROOM` (PNL-09's 1.08 headroom constant), not the raw max -- so a bar exactly at the true max value only reaches `100/1.08 = 92.593%` height, and a half-max bar reaches `46.296%`, not 50%. The "max"/"mid" gridlines were sitting above where a bar of that exact value actually peaks -- correct-*looking* in isolation (which is why the first two fixes both screenshot-verified as "aligned") but wrong once compared against real data. Repositioned `.axis-label.max` to `top: 7.407%` (hangs down from the 92.593%-from-bottom point, same direction the original `top: 0` hung) and `.axis-label.mid` to `bottom: 46.296%; transform: translateY(50%)` (centered on that point) -- both fixed percentages derived from `Y_AXIS_HEADROOM`, not data-dependent, with a comment noting they must be recomputed if that constant changes. `min` (value 0) was already correct and untouched -- headroom only affects non-zero values. Verified by toggling the "Max Size" reference line on and re-screenshotting: the red dashed line, the tallest bar's actual peak, and the "10 KB"/"51" labels now all coincide at the exact same height, on both axes. Full vitest (2377, 118 files) + typecheck + `npm run build` clean a third time. |
