...
| CMP-02 | Side-by-side / Split (H/V) view | P0 | [-] | Side-by-side는 위 CMP-01과 함께 실동작 확인됨(2026-08-19). Split(H/V) 와이프 뷰는 여전히 미구현 — 별개의 단일 캔버스 렌더링 기능이라 이번 패스에서 의도적으로 범위 밖(`DEVELOPMENT_PHASES.md` Phase 7.5 참조) |
...
| CMP-04 | Find First Difference (stream vs stream) | P1 | [ ] | Shares logic with YUVDiff §4.7. 엔진에 알고리즘 자체가 아직 없음(디버그 YUV용 단일 스트림 `find_first_diff_frame`과는 별개) — CMP-01/03이 만든 `get_aligned_frame`/`get_diff_frame` 위에 프레임 루프+임계값 체크만 추가하면 되는 작은 후속 작업으로 스코핑됨(2026-08-19) |
