...
| EVB-01 | One-click Evidence Bundle export (manifest+env+version+selection_state+order_type+backend_fingerprint+plugin_versions+warnings+screenshots, 4 entrypoints, ABI compat policy) | P0 | [-] | Contract: `UX_PARITY_MATRIX.md` §7. Manifest/env/version/selection_state/order_type/backend_fingerprint/warnings all real (Phase 7.6). Screenshots real as of 2026-08-11 (`4d8c904`) -- `bitvue:captureScreenshot` (Electron `capturePage()`) → base64 → sidecar decode → `screenshots/screenshot_NNNN.png`, verified end-to-end via real selftest run. 3/4 entrypoints reachable (MainMenu/BottomBar/ContextMenu; CompareWorkspace toolbar unreachable, that UI is still dead per Phase 7.5). **Diff contract real as of 2026-08-11**: `parity_harness::compare_evidence_bundles` was operating on a disconnected duplicate manifest type (never produced by any real bundle) with an unused config param -- fixed to reuse `export::evidence::EvidenceBundleManifest` (the type actually written to `bundle_manifest.json`) and to gate every field check on `config.compare_fields`; added `compare_evidence_bundle_dirs` (loads `bundle_manifest.json` from two real bundle dirs) and a first real caller, `bitvue evidence-diff <a> <b> [--strict]` (`bitvue-cli`), verified against real exported bundles (differing `order_type` correctly surfaces as `BREAKING`, identical bundles `MATCH`). Still missing: ABI compat policy (no real versioning/compat-checking logic exists at all, `bundle_version` is a hardcoded unread string) -- that needs original design (what gets versioned, an actual compat-rule checker, an alias-mapping mechanism), unlike the diff contract which just needed rewiring existing logic |
