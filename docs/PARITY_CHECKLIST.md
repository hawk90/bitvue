...
| EVB-01 | One-click Evidence Bundle export (manifest+env+version+selection_state+order_type+backend_fingerprint+plugin_versions+warnings+screenshots, 4 entrypoints, ABI compat policy) | P0 | [-] | Contract: `UX_PARITY_MATRIX.md` §7. Manifest/env/version/selection_state/order_type/backend_fingerprint/warnings all real (Phase 7.6). **Screenshots real as of 2026-08-11** (`4d8c904`) -- `bitvue:captureScreenshot` (Electron `capturePage()`) → base64 → sidecar decode → `screenshots/screenshot_NNNN.png`, verified end-to-end via real selftest run. 3/4 entrypoints reachable (MainMenu/BottomBar/ContextMenu; CompareWorkspace toolbar unreachable, that UI is still dead per Phase 7.5). Still missing: ABI compat policy (no real versioning/compat-checking logic exists at all, `bundle_version` is a hardcoded unread string) and the diff contract (algorithm exists in `parity_harness::compare_evidence_bundles` but operates on a disconnected duplicate manifest type, is unwired to any command, and its config param is unused) -- see this session's investigation for exact file:line gaps |
