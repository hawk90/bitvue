...
**검증:** 유닛테스트 27개(신규 2: `select_spatial_block` 성공/잘못된 stream id), 통합 2개 전부 통과. 실제 컴파일된 바이너리에 `select_spatial_block` 단독 요청을 파이프해 `ok:true` + `SelectionUpdated` 확인. `cargo fmt --check`/`cargo check --workspace` 클린.
