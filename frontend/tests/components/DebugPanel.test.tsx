...
const mockFrames = [
  ...
  { frame_index: 1, frame_type: 'P', size: 30000, poc: 1, ref_frames: [0], key_frame: false },
  { frame_index: 2, frame_type: 'P', size: 35000, poc: 2, ref_frames: [0], key_frame: false },
  { frame_index: 3, frame_type: 'B', size: 20000, poc: 3, ref_frames: [1, 2], key_frame: false },
  { frame_index: 4, frame_type: 'B', size: 25000, poc: 4, ref_frames: [2, 3], key_frame: false },
  { frame_index: 5, frame_type: 'P', size: 32000, poc: 5, ref_frames: [0], display_order: 5, coding_order: 3, key_frame: false },
];
...
describe('DebugPanel', () => {
  ...
  it('should not render when not visible', () => {
    const { container } = render(<DebugPanel frames={mockFrames} visible={false} onClose={vi.fn()} />);
...
describe('DebugPanel frame list', () => {
  ...
  it('should limit frame list to 50', () => {
    ...
    render(<DebugPanel frames={largeFrames} visible={true} onClose={vi.fn()} />);
  ...
  it('should show references for frames with refs', () => {
    const { container } = render(<DebugPanel frames={mockFrames} visible={true} onClose={vi.fn()} />);
    ...
    const hasRefsLabel = Array.from(refsLabels).some(el => el.textContent === 'Refs:');
...
describe('DebugPanelToggle', () => {
  ...
  it('should show correct icon when closed', () => {
    const { container } = render(<DebugPanelToggle isOpen={false} onToggle={vi.fn()} />);
  ...
  it('should show correct icon when open', () => {
    const { container } = render(<DebugPanelToggle isOpen={true} onToggle={vi.fn()} />);
  ...
  it('should use React.memo for performance', () => {
    const { rerender } = render(<DebugPanelToggle isOpen={false} onToggle={vi.fn()} />);
...
describe('DebugPanel close button', () => {
  ...
  it('should call onClose when close button clicked', () => {
    ...
    render(<DebugPanel frames={mockFrames} visible={true} onClose={handleClose} />);
...
describe('DebugPanel edge cases', () => {
  it('should handle empty frames array', () => {
    const { container } = render(<DebugPanel frames={[]} visible={true} onClose={vi.fn()} />);
    ...
    const totalFramesLabelIndex = labels.findIndex(el => el.textContent === 'Total Frames');
