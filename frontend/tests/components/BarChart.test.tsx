...
describe("BarChart", () => {
  ...
  it("should sort entries by value descending", () => {
    ...
    const labels = Array.from(rows).map(
      (r) => r.querySelector(".bar-chart-label")?.textContent,
    );
  ...
  it("should use custom max value", () => {
    const { container } = render(
      <BarChart data={{ I: 50, P: 100 }} maxValue={200} />,
    );
  ...
  it("should apply minimum bar height", () => {
    const { container } = render(
      <BarChart data={{ I: 1, P: 1000 }} minBarHeight={5} />,
    );
  ...
  it("should render bar containers", () => {
    ...
    expect(
      container.querySelector(".bar-chart-bar-container"),
    ).toBeInTheDocument();
  });
  ...
  it("should use custom class name", () => {
    const { container } = render(
      <BarChart data={{ I: 10 }} className="custom-chart" />,
    );
  ...
  it("should handle many entries", () => {
    const data = Object.fromEntries(
      Array.from({ length: 20 }, (_, i) => [`Item${i}`, i]),
    );
  ...
  it("should handle zero values", () => {
    ...
    const widths = Array.from(bars).map((bar) =>
      parseFloat((bar as HTMLElement).style.width),
    );
