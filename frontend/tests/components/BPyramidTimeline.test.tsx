...
import { BPyramidTimeline, analyzeTemporalLevels } from '../Filmstrip/views/BPyramidTimeline';
...
const mockFrames: FrameInfo[] = [
  ...
  { frame_index: 1, frame_type: 'B', size: 20000, poc: 4, temporal_id: 2, ref_frames: [0, 8] },
  { frame_index: 2, frame_type: 'B', size: 20000, poc: 2, temporal_id: 1, ref_frames: [0, 4] },
  { frame_index: 3, frame_type: 'B', size: 20000, poc: 6, temporal_id: 2, ref_frames: [4, 8] },
  { frame_index: 4, frame_type: 'B', size: 25000, poc: 1, temporal_id: 0, ref_frames: [0, 2] },
  { frame_index: 5, frame_type: 'B', size: 22000, poc: 5, temporal_id: 2, ref_frames: [2, 6] },
  { frame_index: 6, frame_type: 'B', size: 22000, poc: 3, temporal_id: 1, ref_frames: [2, 4] },
  { frame_index: 7, frame_type: 'B', size: 22000, poc: 7, temporal_id: 2, ref_frames: [6, 8] },
  { frame_index: 8, frame_type: 'I', size: 50000, poc: 8, temporal_id: 0, key_frame: true },
];
...
describe('BPyramidTimeline', () => {
  ...
  it('should render B-Pyramid timeline', () => {
    ...
    expect(document.querySelector('.bpyramid-timeline-container')).toBeInTheDocument();
  });
  ...
  it('should highlight current frame', () => {
    ...
    const currentFrame = document.querySelector('[data-frame-index="4"].selected');
...
describe('BPyramidTimeline edge cases', () => {
  ...
  it('should mark GOP boundary frames', () => {
    ...
    const gopBoundary = document.querySelector('[data-frame-index="0"].gop-boundary');
...
describe('BPyramidTimeline frame circles', () => {
  ...
  it('should handle current frame selection change', () => {
    ...
    const selectedCircle = document.querySelector('[data-frame-index="2"].selected');
    expect(selectedCircle).toBeInTheDocument();

    const previouslySelected = document.querySelector('[data-frame-index="4"].selected');
...
describe('BPyramidTimeline SVG arrows', () => {
  ...
  it('should show arrows only for current frame', () => {
    ...
    const paths = document.querySelectorAll('.bpyramid-arrows-overlay > g > path');
  ...
  it('should sort frames within each level by index', () => {
    ...
    result.levels.forEach(level => {
      for (let i = 1; i < level.frames.length; i++) {
        expect(level.frames[i].frameIndex).toBeGreaterThan(level.frames[i - 1].frameIndex);
      }
    });
  });
