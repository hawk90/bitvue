...
describe("FrameSyntaxTab", () => {
  ...
  it("should render frame properties", () => {
    render(
      <FrameSyntaxTab {...defaultProps} expandedNodes={new Set(["Frame 1"])} />,
    );
...
describe("FrameSyntaxTab reference frames", () => {
  it("should display multiple reference frames", () => {
    ...
    render(
      <FrameSyntaxTab
        frame={frame}
        expandedNodes={new Set(["Frame 5", "Frame 5/ref_frames"])}
        onToggleNode={vi.fn()}
      />,
    );

    const refs = document.querySelectorAll(".syntax-label");
    const refNodes = Array.from(refs).filter(
      (r) => r.textContent === "ref[0]" || r.textContent === "ref[1]",
    );
