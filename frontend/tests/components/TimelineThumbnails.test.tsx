...
import { render, screen, fireEvent, renderWithoutProviders } from '@/test/test-utils';
...
describe('TimelineThumbnails', () => {
  ...
  it('should mark highlighted frame as selected', () => {
    ...
    const selectedThumb = document.querySelector('[data-frame-index="0"].selected');
  ...
  it('should handle KEY frame type', () => {
    const framesWithKey = [
      { frame_index: 0, frame_type: 'KEY', size: 50000, poc: 0, key_frame: true },
    ];
  ...
  it('should update aria-valuenow when highlighted frame changes', () => {
    const { rerender } = renderWithoutProviders(<TimelineThumbnails {...defaultProps} />);

    rerender(<TimelineThumbnails {...defaultProps} highlightedFrameIndex={2} />);
  ...
  it('should have data-frame-index on each thumbnail', () => {
    ...
    expect(document.querySelector('[data-frame-index="0"]')).toBeInTheDocument();
    expect(document.querySelector('[data-frame-index="1"]')).toBeInTheDocument();
    expect(document.querySelector('[data-frame-index="2"]')).toBeInTheDocument();
    expect(document.querySelector('[data-frame-index="3"]')).toBeInTheDocument();
    expect(document.querySelector('[data-frame-index="4"]')).toBeInTheDocument();
  });
...
describe('TimelineThumbnails refs', () => {
  it('should call onFrameRefsChange when refs change', () => {
    ...
    renderWithoutProviders(<TimelineThumbnails {...defaultProps} onFrameRefsChange={onFrameRefsChange} />);
  ...
  it('should update refs when frames length changes', () => {
    ...
    const { rerender } = renderWithoutProviders(
      <TimelineThumbnails {...defaultProps} onFrameRefsChange={onFrameRefsChange} />
    );
...
describe('TimelineThumbnails edge cases', () => {
  ...
  it('should handle lowercase frame types', () => {
    ...
    expect(document.querySelector('[data-frame-index="0"]')).toHaveClass('timeline-bar-i');
    expect(document.querySelector('[data-frame-index="1"]')).toHaveClass('timeline-bar-p');
    expect(document.querySelector('[data-frame-index="2"]')).toHaveClass('timeline-bar-b');
  });
  ...
  it('should handle mixed case frame types', () => {
    ...
    expect(document.querySelector('[data-frame-index="0"]')).toHaveClass('timeline-bar-i');
    expect(document.querySelector('[data-frame-index="1"]')).toHaveClass('timeline-bar-b');
    expect(document.querySelector('[data-frame-index="2"]')).toHaveClass('timeline-bar-p');
  });
  ...
  it('should handle frame types starting with B', () => {
    ...
    expect(document.querySelector('[data-frame-index="0"]')).toHaveClass('timeline-bar-b');
    expect(document.querySelector('[data-frame-index="1"]')).toHaveClass('timeline-bar-b');
    expect(document.querySelector('[data-frame-index="2"]')).toHaveClass('timeline-bar-b');
  });
});
