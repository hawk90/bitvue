...
describe('StreamTreePanel basic rendering', () => {
  ...
  it('should render empty state when no frames loaded', () => {
    ...
    expect(screen.getByText('Open a file to see stream units')).toBeInTheDocument();
  });
...
describe('StreamTreePanel unit selection', () => {
  ...
  it('should call onUnitSelect when unit is clicked', () => {
    ...
    render(
      <StreamTreePanel {...defaultProps} units={mockUnits} onUnitSelect={handleSelect} />
    );

    const firstUnit = screen.getByText(/SEQUENCE_HEADER/).closest('.stream-tree-label');
  ...
  it('should apply selected class to selected unit', () => {
    ...
    render(
      <StreamTreePanel {...defaultProps} units={mockUnits} selectedUnitKey="seq1" />
    );
  ...
  it('should not apply selected class to non-selected units', () => {
    ...
    render(
      <StreamTreePanel {...defaultProps} units={mockUnits} selectedUnitKey="frame1" />
    );

    const firstUnit = screen.getByText(/SEQUENCE_HEADER/).closest('.stream-tree-item');
  ...
  it('should update selection when selectedUnitKey changes', () => {
    ...
    expect(document.querySelector('.stream-tree-item.selected')).toBeInTheDocument();

    rerender(
      <StreamTreePanel {...defaultProps} units={mockUnits} selectedUnitKey="frame1" />
    );

    const selectedItems = document.querySelectorAll('.stream-tree-item.selected');
...
describe('StreamTreePanel filter functionality', () => {
  ...
  it('should show filter options when filter is enabled', () => {
    ...
    expect(screen.getByPlaceholderText('Type or offset...')).toBeInTheDocument();
  ...
  it('should hide filter options when filter is disabled', () => {
    ...
    expect(screen.queryByPlaceholderText('Type or offset...')).not.toBeInTheDocument();
  });
...
describe('StreamTreePanel edge cases', () => {
  ...
  it('should handle undefined onUnitSelect callback', () => {
    ...
    render(<StreamTreePanel {...defaultProps} units={mockUnits} onUnitSelect={undefined} />);
