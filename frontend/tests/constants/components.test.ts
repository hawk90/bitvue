...
describe("ANALYSIS_MODE_LABELS", () => {
  it("should have labels for all analysis modes", () => {
    ...
    expect(ANALYSIS_MODE_LABELS[ANALYSIS_MODES.CODING_FLOW]).toBe(
      "Coding Flow (F2)",
    );
    expect(ANALYSIS_MODE_LABELS[ANALYSIS_MODES.PREDICTION]).toBe(
      "Prediction (F3)",
    );
    expect(ANALYSIS_MODE_LABELS[ANALYSIS_MODES.TRANSFORM]).toBe(
      "Transform (F4)",
    );
    ...
    expect(ANALYSIS_MODE_LABELS[ANALYSIS_MODES.REFERENCE_FRAMES]).toBe(
      "Reference Frames (F7)",
    );
  });
});
