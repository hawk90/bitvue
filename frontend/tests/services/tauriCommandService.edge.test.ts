...
describe("TauriCommandService edge cases", () => {
  ...
  describe("abnormal command names", () => {
    it("should handle empty command name", async () => {
      ...
      await expect(TauriCommandService.invoke("")).rejects.toThrow(
        TauriCommandError,
      );
    });
    ...
    it("should handle very long command names", async () => {
      ...
      await expect(TauriCommandService.invoke(longCommand)).rejects.toThrow(
        TauriCommandError,
      );
    });
  ...
  describe("retry logic edge cases", () => {
    ...
    it("should handle large retry count", async () => {
      ...
      const result = await TauriCommandService.invoke(
        "test",
        {},
        { retry: 100 },
      );
  ...
  describe("error handling edge cases", () => {
    it("should handle Error objects", async () => {
      ...
      await expect(TauriCommandService.invoke("test")).rejects.toThrow(
        TauriCommandError,
      );
    });
  ...
  describe("options edge cases", () => {
    ...
    it("should handle invalid option values", async () => {
      ...
      await TauriCommandService.invoke(
        "test",
        {},
        { log: "yes" as unknown as boolean },
      );
      // @ts-expect-error - Testing invalid inputs
