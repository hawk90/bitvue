...
export function isError(value: unknown): value is Error {
  return value instanceof Error || (isObject(value) && 'name' in value && 'message' in value);
}
...
export function isTauriSuccess<T = unknown>(value: unknown): value is TauriSuccess<T> {
 ...
 */
export function isValidFrameIndex(value: unknown, maxFrames: number): value is number {
  return isNumber(value) && Number.isInteger(value) && value >= 0 && value < maxFrames;
}
