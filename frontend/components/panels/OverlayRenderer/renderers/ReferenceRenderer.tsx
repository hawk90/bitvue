...
export const ReferenceOverlay = memo(function ReferenceOverlay({
  ...
  const isKeyFrame = frameType === 'I' || frameType === 'KEY' || frameType === 'INTRA';
  ...
  if (isKeyFrame) {
    ...
    ctx.fillText('Key frame - no references', infoX + 10, textY + lineHeight * 2);
  } else {
    ctx.fillText(`References: ${refFrames.length} frame${refFrames.length !== 1 ? 's' : ''}`, infoX + 10, textY + lineHeight * 2);
    if (refFrames.length > 0) {
      ctx.fillStyle = getCssVar('--text-secondary') || '#aaaaaa';
      ctx.fillText(`[${refFrames.join(', ')}]`, infoX + 10, textY + lineHeight * 3);
    }
  }
});
