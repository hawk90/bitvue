...
export const RDCurvesPanel = memo(function RDCurvesPanel() {
  ...
  const [selectedMetric, setSelectedMetric] = useState<'PSNR' | 'SSIM' | 'VMAF'>('PSNR');
    ...
    <div className="rd-curves-panel p-4">
      ...
      {curves.length > 0 ? (
        ...
        <div className="mb-4 p-8 border rounded bg-gray-50 text-center text-text-secondary">
          ...
          <p className="text-sm">Click "Load Demo Data" to see example RD curves, or load your own encoding data</p>
        </div>
      )}
      ...
      {bdRateResult && (
        <div className="p-4 border rounded bg-green-50">
          <h4 className="font-semibold mb-3 text-green-800">BD-Rate Calculation Results</h4>
          <div className="grid grid-cols-3 gap-4">
            ...
            <div>
              ...
              <div className={`font-bold text-lg ${bdRateResult.bd_rate < 0 ? 'text-green-600' : 'text-red-600'}`}>
                {bdRateResult.bd_rate > 0 ? '+' : ''}{bdRateResult.bd_rate.toFixed(2)}%
              </div>
            </div>
            <div>
              <div className="text-sm text-gray-600">BD-PSNR</div>
              <div className="font-bold text-lg text-blue-600">
                {bdRateResult.bd_psnr > 0 ? '+' : ''}{bdRateResult.bd_psnr.toFixed(2)} dB
              </div>
            </div>
      ...
      <div className="mt-4 text-sm text-gray-500">
        <p><strong>BD-Rate</strong> (Bjøntegaard Delta Rate) measures the bitrate savings between two codecs for equivalent quality.</p>
        <p className="mt-1">Negative values indicate the test codec is more efficient (lower bitrate for same quality).</p>
        <p className="mt-1">Load multiple RD curves to compare encoder performance across different bitrates.</p>
      </div>
    </div>
  );
});
